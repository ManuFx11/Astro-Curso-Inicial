// To parse this data:
//
//   import { Convert, SpaceAPIRespond } from "./file";
//
//   const spaceAPIRespond = Convert.toSpaceAPIRespond(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.

export interface SpaceAPIRespond {
    docs:          Doc[];
    totalDocs:     number;
    offset:        number;
    limit:         number;
...
export class Convert {
    public static toSpaceAPIRespond(json: string): SpaceAPIRespond {
        return cast(JSON.parse(json), r("SpaceAPIRespond"));
    }

    public static spaceAPIRespondToJson(value: SpaceAPIRespond): string {
        return JSON.stringify(uncast(value, r("SpaceAPIRespond")), null, 2);
    }
}
